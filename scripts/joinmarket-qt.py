...
from jmui import Ui_OpenWalletDialog
...
class JMWalletTab(QWidget):
    ...
    def updateWalletInfo(self, walletinfo=None):
        max_mix_depth_count = jm_single().config.getint("GUI", "max_mix_depth")
        ...
        for mix_depth in range(max_mix_depth_count):
            if walletinfo:
                mdbalance = mbalances[mix_depth]
            else:
                mdbalance = "{0:.8f}".format(0)
            m_item = QTreeWidgetItem(["Mixdepth " + str(mix_depth) + " , balance: " +
                                      mdbalance, '', '', '', ''])
            self.walletTree.addChild(m_item)
            
            # if expansion states existed, reinstate them:
            if len(previous_expand_states) == max_mix_depth_count:
                m_item.setExpanded(previous_expand_states[mix_depth][0])
            ...
            for forchange in [0, 1]:
                ...
                if walletinfo and heading == "EXTERNAL":
                    heading_end = ' ' + xpubs[mix_depth][forchange]
                    heading += heading_end
                seq_item = QTreeWidgetItem([heading, '', '', '', ''])
                m_item.addChild(seq_item)

                # by default, the external addresses of mixdepth 0 is expanded
                should_expand = mix_depth == 0 and not forchange
                if not walletinfo:
                    item = QTreeWidgetItem(['None', '', '', ''])
                    seq_item.addChild(item)
                else:
                    for j in range(len(rows[mix_depth][forchange])):
                        item = QTreeWidgetItem(rows[mix_depth][forchange][j])
                        item.setFont(0, QFont(MONOSPACE_FONT))
                        if rows[mix_depth][forchange][j][3] != 'new':
                            item.setForeground(3, QBrush(QColor('red')))
                        # by default, if the balance is non zero, it is also expanded
                        if float(rows[mix_depth][forchange][j][2]) > 0:
                            should_expand = True
                        seq_item.addChild(item)
                # Remember user choice, if expansion states existed, reinstate them:
                if len(previous_expand_states) == max_mix_depth_count:
                    should_expand = previous_expand_states[mix_depth][forchange+1]
